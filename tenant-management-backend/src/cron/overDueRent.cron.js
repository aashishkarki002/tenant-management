...
export const startOverDueRentCron = (io) => {
  ...
  // Validate ADMIN_ID
  if (!ADMIN_ID) {
    console.error("❌ SYSTEM_ADMIN_ID is not set in environment variables");
    return;
  }

  console.log(`✅ Overdue rent cron job started with ADMIN_ID: ${ADMIN_ID}`);
  ...
  cron.schedule(
    ...
    async () => {
      try {
        ...
        console.log(`Found ${overdueRents.length} overdue rent(s)`);
          ...
          try {
            ...
            if (remaining > 0) {
              ...
              const notificationData = {
                ...
                data: {
                  ...notification.data,
                  rentId: notification.data.rentId.toString(),
                },
                createdAt: notification.createdAt,
              };

              const adminRoom = `admin:${ADMIN_ID}`;
              io.to(adminRoom).emit("rent:overdue", notificationData);

              notificationCount++;
            }
          } catch (error) {
            console.error(
              `❌ Error creating notification for rent ${rent._id}:`,
              error.message
            );
          }
        }
        ...
        console.log(
          `✅ Cron job completed. Created ${notificationCount} notification(s)`
        );
      } catch (error) {
        console.error("❌ Error in overdue rent cron job:", error);
      }
    },
    {
      scheduled: true,
