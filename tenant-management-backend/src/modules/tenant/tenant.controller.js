  ...
  export default async function createTenant(req, res) { 
    try {
      ...
      if (!pdfFile) {
        ...
        return res.status(400).json({ success: false, message: "PDF agreement file is required" });
      }
  ...
  export const getTenants = async (req, res) => {
    try {
      const tenants = await Tenant.find({ isDeleted: false }).populate('property').populate('block').populate('innerBlock');
      res.status(200).json({ success: true, tenants });
    } catch (error) {
      console.log(error);
      res.status(500).json({ success: false, message: "Error fetching tenants", error: error });
    }
  }
  ...
  export const deleteTenant = async (req, res) => {
    try {
      const tenantId = req.params.id;
      const softDeletedTenant = await Tenant.findByIdAndUpdate(tenantId, { isDeleted: true }, { new: true });
      if (!softDeletedTenant) {
        return res.status(404).json({ success: false, message: "Tenant not found" });
      }
      res.status(200).json({ success: true, message: "Tenant deleted successfully", tenant: softDeletedTenant });
    } catch (error) {
      console.log(error);
  ...
  export const restoreTenant = async (req, res) => {
    try {
      const tenantId = req.params.id;
      const restoredTenant = await Tenant.findByIdAndUpdate(tenantId, { isDeleted: false }, { new: true });
      if (!restoredTenant) {
        return res.status(404).json({ success: false, message: "Tenant not found" });
      }
      res.status(200).json({ success: true, message: "Tenant restored successfully", tenant: restoredTenant });
    } catch (error) {
      console.log(error);
      res.status(500).json({ success: false, message: "Error restoring tenant", error: error.message });
    }
  };
...
if (search) {
  filters.name = { $regex: search, $options: "i" };
      ...
      const tenants = await Tenant.find(filters).populate('property').populate('block').populate('innerBlock');
      res.status(200).json({ success: true, tenants });
    } catch (error) {
      console.log(error);
      res.status(500).json({ success: false, message: "Error searching tenants", error: error.message });
    }
  };