...
const accessToken = generateAccessToken({ id: admin._id, role: admin.role });
const refreshToken = generateRefreshToken({ id: admin._id, role: admin.role });
        return { success: true, message: "Password changed successfully", accessToken, refreshToken };
    } catch (error) {
        console.error("Change password error:", error);
        return { success: false, message: "Change password failed" , error: error.message };
    }
}
