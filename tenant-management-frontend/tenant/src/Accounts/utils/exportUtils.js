...
function drawHeader(doc, title, subtitle, period, entityName) {
  ...
  doc.text(`Generated: ${todayLabel()}`, PAGE_W - MARGIN_R, y, {
    align: "right",
  });
...
function addFooter(doc) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Page ${i} of ${pageCount}`, PAGE_W / 2, PAGE_H - 5, {
      align: "center",
    });
  }
}
 ...
 * Columns: Date (BS), Description, Account, Debit (RS), Credit (RS), Running Balance (RS)
 *
...
export function exportLedgerCSV(
  ...
  rows.push([
    ...
    "",
    "",
    (totals.netCashFlow ?? 0).toFixed(2),
  ]);
  ...
  triggerDownload(
    blob,
    `Ledger_${sanitizeFilename(filterLabel)}_${todayLabel()}.csv`,
  );
}
...
export function exportLedgerPDF(
  ...
  for (const entry of entries) {
    ...
    if (entry.debit) {
      ...
      doc.text(fmtMoney(entry.debit, true), cols[2].x + cols[2].w, y, {
        align: "right",
      });
      doc.setTextColor(50, 50, 50);
    } else {
    ...
    if (entry.credit) {
      ...
      doc.text(fmtMoney(entry.credit, true), cols[3].x + cols[3].w, y, {
        align: "right",
      });
      doc.setTextColor(50, 50, 50);
    } else {
    ...
    if (bal !== undefined) {
      doc.setFont("helvetica", "bold");
      doc.text(fmtMoney(bal, true), cols[4].x + cols[4].w, y, {
        align: "right",
      });
      doc.setFont("helvetica", "normal");
    } else {
  ...
  for (const row of summaryRows) {
    ...
    doc.text(`Rs ${fmtMoney(row.value, true)}`, PAGE_W - MARGIN_R, y, {
      align: "right",
    });
    doc.setTextColor(50, 50, 50);
    y += 5.5;
...
export function exportBalanceSheetPDF(
  ...
  if (retainedEarnings) {
    yR = drawAccountRow(
      ...
      retainedEarnings.balance?.paisa ?? 0,
      2,
    );
  }
  yR += 2;
  ...
  doc.text(`Rs ${fmtMoney(totalAssets?.paisa ?? 0)}`, COL_MID - 4, finalY + 6, {
    align: "right",
  });

  doc.text("Total Liabilities + Equity", COL_R_X, finalY + 6);
  doc.text(
    `Rs ${fmtMoney(totalLiabilitiesAndEquity?.paisa ?? 0)}`,
    PAGE_W - MARGIN_R,
    finalY + 6,
    { align: "right" },
  );
