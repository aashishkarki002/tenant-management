...
export default function CamReconciliationTab() {
  ...
  const { data, loading, error, refetch } = useCamReconciliation(selectedEntity?.id ?? null, nepaliYear);
            ...
            <table className="w-full text-sm">
                ...
                {(data.tenants ?? []).map((t) => {
                  ...
                  return (
                    <tr key={t.tenantId} className="border-b border-[var(--color-border)]/30 hover:bg-[var(--color-surface-hover)] transition-colors">
                      ...
                      <td className="py-2 pr-4 text-right text-xs font-mono">{fmtPaisa(t.billedPaisa)}</td>
                      <td className="py-2 pr-4 text-right text-xs font-mono text-[var(--color-success)]">{fmtPaisa(t.paidPaisa)}</td>
              ...
              <tfoot>
                <tr className="border-t-2 border-[var(--color-border)] font-bold">
                  ...
                  <td className="pt-3 pb-2 text-right text-sm font-mono">{fmtPaisa(data.totalBilledPaisa ?? 0)}</td>
                  <td className="pt-3 pb-2 text-right text-sm font-mono text-[var(--color-success)]">{fmtPaisa(data.totalPaidPaisa ?? 0)}</td>
                  <td className="pt-3 pb-2 text-right text-sm font-mono" style={{ color: "var(--color-danger)" }}>
                    {fmtPaisa((data.totalBilledPaisa ?? 0) - (data.totalPaidPaisa ?? 0))}
                  </td>
                </tr>
              </tfoot>
            </table>
