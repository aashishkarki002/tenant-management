...
// ═══════════════════════════════════════════════════════════════════════════════
function StaffTab({ staff, onRefresh }) {
  ...
  const deleteStaff = async (id) => {
    try {
      ...
      if (res.data.success) { toast.success(res.data.message || "Removed"); onRefresh?.(); }
  ...
  return (
    <div className="space-y-4">
      <Card className="border-border">
        <CardHeader className="pb-4 border-b border-border">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              <div className="w-6 h-6 rounded-md bg-slate-900 flex items-center justify-center"><Users className="w-3.5 h-3.5 text-white" /></div>
              Staff Members
              <Badge variant="secondary" className="text-[11px] font-semibold">{staff?.length ?? 0}</Badge>
            </CardTitle>
            <div className="flex gap-2">
              <Input placeholder="Search…" value={search} onChange={(e) => setSearch(e.target.value)} className="h-8 w-36 text-xs" />
        ...
        <CardContent className="pt-4 space-y-2">
          {filtered.length === 0 ? (
            <div className="text-center py-10 border-2 border-dashed border-border rounded-xl">
              ...
              <p className="text-sm font-medium text-muted-foreground">{search ? "No matches" : "No staff yet"}</p>
            </div>
          ) : filtered.map((m) => (
            <div key={m._id} className="flex flex-col sm:flex-row sm:items-center justify-between rounded-xl border border-border bg-secondary/30 px-4 py-3 gap-3 hover:bg-secondary/60 transition-colors">
              <div className="flex items-center gap-3">
                <Avatar className="w-9 h-9 border border-border">
                  <AvatarImage src={m?.profilePicture} />
                  <AvatarFallback className="bg-slate-100 text-slate-600 text-xs font-bold">{getInitials(m?.name)}</AvatarFallback>
                </Avatar>
                <div>
              ...
              <div className="flex items-center gap-3 sm:ml-auto flex-wrap">
                <Badge variant="outline" className="text-[10px] capitalize font-semibold">{m?.role}</Badge>
                {m?.phone && (
                  <a href={`tel:${m.phone}`}>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>Access will be permanently revoked.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                      <AlertDialogAction onClick={() => deleteStaff(m._id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Remove</AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
                </AlertDialog>
              </div>
      ...
      <AddStaffDialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) onRefresh?.(); }} />
    </div>
  );
}
