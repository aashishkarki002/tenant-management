...
/**
 * Current BS month (1–12) and year; safe fallback for edge-case failures.
 * @returns {{ month: number, year: number }}
 */
export function getCurrentNepaliMonthYear() {
  try {
    return { month: getCurrentNepaliMonth(), year: getCurrentNepaliYear() };
  } catch {
...
/**
 * Month options with "Name (n)" labels for selects (e.g. "Baisakh (1)").
 *
 * @param {object} [opts]
 * @param {number} [opts.quarter]
 * @param {'en'|'np'} [opts.lang='en']
 * @returns {{ value: number, label: string }[]}
 */
export function getNepaliMonthOptionsWithNumber({ quarter, lang = "en" } = {}) {
  return getNepaliMonthOptions({ quarter, lang }).map((o) => ({
    ...o,
...
// ============================================================================
// BILLING PERIOD (BS month/year for ledgers, electricity, etc.)
// ============================================================================

/**
 * Current Nepali billing period (1-based month).
 * @returns {{ nepaliYear: number, nepaliMonth: number }}
 */
export function getCurrentBillingPeriod() {
  return {
    nepaliYear: getCurrentNepaliYear(),
    nepaliMonth: getCurrentNepaliMonth(),
  };
}

/**
 * @param {{ nepaliYear: number, nepaliMonth: number }} period
 * @returns {string} e.g. "Mangsir 2082"
 */
export function labelForPeriod({ nepaliYear, nepaliMonth }) {
  const name = NEPALI_MONTH_NAMES[nepaliMonth - 1];
  return `${name} ${nepaliYear}`;
}

/**
 * Composite key for a billing period: "2082-08"
 */
...
/**
 * Last `count` billing periods as select options, newest first.
...
/**
 * Month options for a standalone month Select (1-based values).
 * @param {'en'|'np'} [lang='en']
 */
...
/**
 * Year options, newest first (default floor 2078 to match historical UI).
...
/**
 * Display without comma: "15 Ashwin 2081" (matches legacy checklist / rent strings).
 *
 * @param {NepaliDate|{year:number,month:number,day:number}} date - month 1-based
 * @param {'en'|'np'} [lang='en']
 * @returns {string}
 */
export function formatNepaliDisplayNoComma(date, lang = "en") {
  let year, month1, day;
...
/**
 * Safe parse of BS "YYYY-MM-DD". Returns null if invalid.
 *
 * @param {string} isoString
 * @returns {{ year: number, month: number, day: number } | null} month 1-based
 */
export function tryParseNepaliISO(isoString) {
  if (!isoString || typeof isoString !== "string") return null;
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(isoString.trim());
  if (!m) return null;
  const year = parseInt(m[1], 10);
  const month = parseInt(m[2], 10);
  const day = parseInt(m[3], 10);
  if (!isValidNepaliYearMonth(year, month)) return null;
  try {
...
/**
 * AD calendar "YYYY-MM-DD" → BS "YYYY-MM-DD" (padded). Empty string on failure.
 *
 * @param {string} adIso
 * @returns {string}
 */
export function adIsoToBsIso(adIso) {
  if (!adIso || typeof adIso !== "string") return "";
  const datePart = adIso.slice(0, 10);
  const parts = datePart.split("-").map(Number);
  if (parts.length !== 3 || parts.some((n) => Number.isNaN(n))) return "";
  const [enYear, enMonthHuman, enDay] = parts;
  try {
    ...
    return _formatISO(npYear, npMonth0 + 1, npDay);
  } catch {
    return "";
  }
}
...
/**
 * AD "YYYY-MM-DD" → backend fields `{ nepaliDate, nepaliMonth, nepaliYear }`.
 * Omits or empty `adDateString` uses today's UTC date prefix (same as previous hook).
 *
 * @param {string} [adDateString]
 * @returns {{ nepaliDate: string, nepaliMonth: number, nepaliYear: number }}
 */
export function adIsoToNepaliApiFields(adDateString) {
...
/**
 * English date → "DD Month YYYY" BS label, or "N/A" (legacy `formatNepali.toNepaliDate`).
 *
...
export function toNepaliDate(englishDate) {
  ...
  if (englishDate instanceof Date) {
    ...
    month0 = englishDate.getMonth();
    day = englishDate.getDate();
  } else if (typeof englishDate === "string") {
    const datePart = englishDate.slice(0, 10);
    const parts = datePart.split("-").map(Number);

    if (parts.length !== 3 || parts.some((n) => Number.isNaN(n))) return "N/A";

    [year, month0, day] = parts;
    month0 = month0 - 1;
  } else {
    return "N/A";
  }
  ...
  if (month0 < 0 || month0 > 11 || day < 1 || day > 31) {
    console.warn(
      `Invalid date values: year=${year}, month=${month0 + 1}, day=${day}`,
      englishDate,
    );
    return "N/A";
  }
  ...
  try {
    ...
    if (npMonth0 < 0 || npMonth0 > 11) {
      console.warn(`Invalid Nepali month index: ${npMonth0}`, englishDate);
      return "N/A";
    }

    const monthName = NEPALI_MONTH_NAMES[npMonth0] || "Unknown";
    ...
    if (
      ...
      console.warn(
        `Date out of range for conversion: ${year}-${month0 + 1}-${day}`,
        englishDate,
      );
    } else {
