...
import { ElectricityKpiStrip }      from "./components/ElectricityKpiStrip";
import { ConsumptionTrendChart }     from "./components/ConsumptionTrendChart";
import { NeaBillPanel }             from "./components/NeaBillPanel";
...
export default function ElectricityPage() {
  ...
  const apiFilters = useMemo(() => ({
    ...
    blockId:     filterValues.blockId !== "all" ? filterValues.blockId : undefined,
  ...
  }), [propertyIdFromBlock, filterValues, activeTab, searchQuery]);
  ...
  const handleExportReport = useCallback(() => {
    ...
    rows.push("");
    rows.push("Meter Type,Unit Name,Previous,Current,Consumption,Status,Trend");

    readings.forEach((record, index) => {
      const name       = record.unit?.name || record.unit?.unitName || record.subMeter?.name || `Row ${index + 1}`;
      const previous   = Number(record.previousReading) || 0;
      const current    = Number(record.currentReading)  || 0;
      const consumption= Number(record.unitsConsumed)   || getConsumption(record);
      const trend      = getTrendPercent(consumption, previous);
    ...
    () => (
      <div className="flex w-full items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button onClick={handleOpenAddReading}>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      {/* ── Top bar: page header + KPI strip + filters ───────────────────── */}
      <div className="flex shrink-0 flex-col gap-3 px-5 pb-3 pt-4">

        {/* Page header */}
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold tracking-tight">Electricity</h1>
        ...
        {/* KPI strip — Total Consumption hero */}
        <ElectricityKpiStrip
          grouped={grouped}
          summary={summary}
      ...
      >
        {/* Left column — chart + table (scrollable) */}
        <div
          className="flex min-h-0 flex-col gap-3"
          style={{ overflowY: "auto", height: "100%" }}
        >
          {/* Consumption trend chart + meter coverage */}
          <ConsumptionTrendChart
            neaBills={neaBills}
            currentPeriod={{ year: filterValues.year, month: filterValues.month }}
            grouped={grouped}
          />
            ...
            <div className="flex items-center justify-between rounded-md border bg-muted/40 px-4 py-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm">
                  ...
                  result{readings.length !== 1 ? "s" : ""} for{" "}
        ...
        {/* Right column — sticky NEA Bill reference panel */}
